...
// actually fully settled today. To keep aging trustworthy, this shared
// helper:
...
// Returns a Map keyed by contractor name -> { amount, maxAge, rows }.
function groupInvoiceAgingByContractor(invoices, contractors, now) {
  ...
  for (const [contractorName, rows] of byContractor.entries()) {
    ...
    const scaledRows = rows.map((r) => ({ amount: r.amount * scale, age: r.age }));
    const amount = scaledRows.reduce((sum, r) => sum + r.amount, 0);
    const maxAge = Math.max(...scaledRows.map((r) => r.age));

    result.set(contractorName, { amount, maxAge, rows: scaledRows });
  }
...
export function computeAging(invoices, contractors = [], now = new Date()) {
  const perContractor = groupInvoiceAgingByContractor(invoices, contractors, now);
  const buckets = AGING_BUCKETS.map((b) => ({ ...b, amount: 0, count: 0 }));

  for (const entry of perContractor.values()) {
    for (const row of entry.rows) {
      const bucket = buckets.find((b) => row.age >= b.min && row.age <= b.max) || buckets[buckets.length - 1];
  ...
  const over90 = Array.from(perContractor.entries())
    .filter(([, e]) => e.maxAge >= 91)
    .map(([contractorName, e]) => ({ contractorName, amount: e.amount, maxAge: e.maxAge }))
    .sort((a, b) => b.amount - a.amount);

  return { buckets, totalOutstanding, over90 };
}

// Per-contractor aging detail: every contractor with a real outstanding
// balance, tagged with how many days their oldest unpaid invoice has been
// sitting and which aging bucket that falls into. This is the "show me
// every customer with a bill pending more than N days" view - the
// per-contractor breakdown behind the aging bucket totals above.
export function computeContractorAgingDetail(invoices, contractors, now = new Date()) {
  const perContractor = groupInvoiceAgingByContractor(invoices, contractors, now);
  const contractorByName = new Map(contractors.map((c) => [c.name, c]));

  const rows = Array.from(perContractor.entries()).map(([contractorName, e]) => {
    const bucket =
      AGING_BUCKETS.find((b) => e.maxAge >= b.min && e.maxAge <= b.max) ||
      AGING_BUCKETS[AGING_BUCKETS.length - 1];
    const contractor = contractorByName.get(contractorName);
    return {
      name: contractorName,
      phone: contractor?.phone || "",
      balance: e.amount,
      daysPending: e.maxAge,
...
// Canonical category keys for each follow-up reason, used for the
// breakdown summary and category filter tabs (human-readable "reasons"
// stay separate since one contractor can carry multiple).
export const FOLLOW_UP_CATEGORIES = [
  { key: "hold", label: "On Hold" },
  { key: "over-limit", label: "Over Limit" },
  { key: "aging-90", label: "90+ Days Overdue" },
  { key: "stalled", label: "No Payment 6mo+" },
];

export function buildDailyFollowUpList({ healthResults, aging, collectionEstimates }) {
  const byContractor = new Map();

  function addReason(name, category, reason, severity, action) {
    if (!byContractor.has(name)) {
      byContractor.set(name, { name, reasons: [], actions: [], categories: [], severity: "medium" });
    }
  ...
  for (const c of healthResults) {
    if (c.hold && c.totalBalance > 0) {
      addReason(c.name, "hold", "On hold with balance", "high", "Resolve hold or collect balance");
    }
    if (c.limitStatus === "over") {
      addReason(c.name, "over-limit", "Over credit limit", "high", "Collect before extending further credit");
    }
  }

  for (const entry of aging.over90) {
    addReason(entry.contractorName, "aging-90", "90+ days overdue", "high", "Priority collection call");
  }

  for (const est of collectionEstimates) {
    if (est.stalled && est.balance > 0) {
      addReason(est.name, "stalled", "No payment in 6+ months", "medium", "Reconnect / confirm status");
    }
  }
...
// Summary counts + total balance per category, for the breakdown panel
// at the top of the follow-ups page.
export function buildFollowUpBreakdown(followUpList) {
  return FOLLOW_UP_CATEGORIES.map((cat) => {
    const items = followUpList.filter((item) => item.categories.includes(cat.key));
    return {
      key: cat.key,
      label: cat.label,
      count: items.length,
