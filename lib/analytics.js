...
export function computeAging(invoices, now = new Date()) {
  const buckets = AGING_BUCKETS.map((b) => ({ ...b, amount: 0, count: 0 }));
  const contractorAgeMax = new Map(); // contractorName -> { amount, maxAge }
  ...
  for (const inv of invoices) {
    ...
    const entry = contractorAgeMax.get(inv.contractorName) || { contractorName: inv.contractorName, amount: 0, maxAge: 0 };
    entry.amount += inv.balanceAmount;
    entry.maxAge = Math.max(entry.maxAge, age);
    contractorAgeMax.set(inv.contractorName, entry);
  }
