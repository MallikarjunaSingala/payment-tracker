...
// Flat, cross-contractor dataset for the analytics dashboard: every
// contractor's aggregate summary (same as getContractors()) plus every
// invoice and payment row tagged with its contractor name, so trends,
// aging, and forecasts can be computed across the whole book at once.
export async function getAnalyticsSnapshot() {
  const { customers, invoices, payments } = await loadAllData();

  const projectToContractor = new Map();
  for (const c of customers) {
    const projectName = c.CustomerName;
    const contractorName = (c.Client || "").trim() || "Unassigned";
    projectToContractor.set(projectName, contractorName);
  }

  const flatInvoices = invoices.map((inv) => ({
    ...mapInvoiceRow(inv),
    projectName: inv.CustomerName,
    contractorName: projectToContractor.get(inv.CustomerName) || "Unassigned",
  }));

  const flatPayments = payments.map((p) => ({
    ...mapPaymentRow(p),
    projectName: p.CustomerName,
    contractorName: projectToContractor.get(p.CustomerName) || "Unassigned",
  }));

  const contractors = await getContractors();

  return { contractors, invoices: flatInvoices, payments: flatPayments };
}
